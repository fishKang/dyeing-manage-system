...
@Service
public class UpdateProcessingDetailsAPS extends AbstractTradeAPS{
    @Autowired
    ...
    public Map<String, Object> mainProcess(Map<String, Object> map, Logger logger) {
        logger.info("UpdateProcessingDetailsAPS-更新明细开始，入参为："+ JSONUtil.toJsonStr(map));
        try{
            DmsProcessingdtlVO dmsProcessingdtlVO = initInputParam(map,logger);
            dmsProcessingdtlManageBOS.updateProcessingdtl(dmsProcessingdtlVO,logger);
            logger.info("UpdateProcessingDetailsAPS-更新明细结束，出参为："+ dmsProcessingdtlVO.toString());
