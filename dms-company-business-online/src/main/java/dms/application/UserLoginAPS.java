...
@Service
public class UserLoginAPS extends AbstractTradeAPS{
    @Autowired
    DmsUserManageBOS dmsUserManageBOS;
    @Override
    public Map<String, Object> mainProcess(Map<String, Object> map, Logger logger) {
        logger.info("UserLoginAPS-用户登录开始，入参为："+ JSONUtil.toJsonStr(map));
        try{
            DmsUserVO dmsUserVO = initInputParam(map,logger);
            dmsUserManageBOS.userLogin(dmsUserVO,logger);
            logger.info("UserLoginAPS-用户登录结束，出参为："+ JSONUtil.toJsonStr(map));
            return ConvertorToMap.convertorSuccessMap(BeanUtil.beanToMap(dmsUserVO.getDmsUserBO()));
